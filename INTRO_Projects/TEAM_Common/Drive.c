...
static void DriveTask(void *pvParameters) {
  ...
  for(;;) {
    ...
    FRTOS1_vTaskDelayUntil(&xLastWakeTime, 5/portTICK_PERIOD_MS); // läuft mit 5 ms
  } /* for */
}
...
void DRV_Init(void) {
  ...
  if (xTaskCreate(DriveTask, "Drive", 400/sizeof(StackType_t), NULL, tskIDLE_PRIORITY+5, NULL) != pdPASS) {
    for(;;){} /* error */
  }
}
